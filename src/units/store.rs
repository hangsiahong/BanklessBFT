...
/// to the Terminal. We refer to the documentation https://hangsiahong.github.io/BanklessBFT/internals.html
...
impl<'a, H: Hasher, D: Data, KB: KeyBox> UnitStore<'a, H, D, KB> {
    ...
    pub(crate) fn mark_forker(&mut self, forker: NodeIndex) -> Vec<SignedUnit<'a, H, D, KB>> {
        if self.is_forker[forker] {
            warn!(target: "BanklessBFT-unit-store", "Trying to mark the node {:?} as forker for the second time.", forker);
        }
    ...
    pub(crate) fn add_unit(&mut self, su: SignedUnit<'a, H, D, KB>, alert: bool) {
        ...
        if alert {
            trace!(target: "BanklessBFT-unit-store", "Adding unit with alert {:?}.", su.as_signable());
            assert!(
        ...
        if self.contains_hash(&hash) {
            ...
            trace!(target: "BanklessBFT-unit-store", "A unit ignored as a duplicate {:?}.", su.as_signable());
            return;
        }
