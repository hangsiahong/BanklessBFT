    ...
    loop {
        ...
        futures::select! {
            unit = incoming_parents.next() => match unit {
                ...
                None => {
                    info!(target: "BanklessBFT-creator", "Incoming parent channel closed, exiting.");
                    return Err(());
                }
            },
            _ = &mut delay => {
                if can_create {
                    warn!(target: "BanklessBFT-creator", "Delay passed despite us not waiting for it.");
                }
                can_create = true;
                delay = Delay::new(Duration::from_secs(30 * 60)).fuse();
            },
            _ = exit => {
                info!(target: "BanklessBFT-creator", "Received exit signal.");
                return Err(());
            },
        }
    }
...
pub async fn run<H: Hasher>(
    ...
    let starting_round = futures::select! {
        maybe_round =  starting_round => match maybe_round {
            Ok(round) => round,
            Err(e) => {
                error!(target: "BanklessBFT-creator", "Starting round not provided: {}", e);
                return;
            }
        },
        _ = &mut exit => return,
    };
    debug!(target: "BanklessBFT-creator", "Creator starting from round {}", starting_round);
    for round in starting_round..max_round {
        ...
        {
            warn!(target: "BanklessBFT-creator", "Notification send error: {}. Exiting.", e);
            return;
        }
    }
    warn!(target: "BanklessBFT-creator", "Maximum round reached. Not creating another unit.");
}
