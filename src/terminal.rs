...
///
/// We also refer to the documentation https://hangsiahong.github.io/BanklessBFT/internals.html
...
impl<H: Hasher> Terminal<H> {
    ...
    fn update_on_store_add(&mut self, u: Unit<H>) {
        ...
        if let Entry::Vacant(entry) = self.unit_by_coord.entry((u_round, pid)) {
            ...
            debug!(target: "BanklessBFT-terminal", "Received a fork at round {:?} creator {:?}", u_round, pid);
        }
        ...
        if u_round == 0 {
            ...
            if !coords_to_request.is_empty() {
                trace!(target: "BanklessBFT-terminal", "{:?} Missing coords {:?}", self.node_id, coords_to_request);
    ...
    fn update_on_wrong_hash_response(&mut self, u_hash: H::Hash, p_hashes: Vec<H::Hash>) {
        ...
        if u.status != UnitStatus::WrongControlHash {
            trace!(target: "BanklessBFT-terminal", "{:?} Received parents response without it being expected for {:?}. Ignoring.", self.node_id, u_hash);
            return;
        }
        for (counter, i) in u.unit.control_hash().parents().enumerate() {
            u.parents.insert(i, p_hashes[counter]);
        }
        trace!(target: "BanklessBFT-terminal", "{:?} Updating parent hashes for wrong control hash unit {:?}", self.node_id, u_hash);
        u.n_miss_par_decoded = NodeCount(0);
        self.inspect_parents_in_dag(&u_hash);
    }

    fn add_to_store(&mut self, u: Unit<H>) {
        trace!(target: "BanklessBFT-terminal", "{:?} Adding to store {:?} round {:?} index {:?}", self.node_id, u.hash(), u.round(), u.creator());
    ...
    fn inspect_parents_in_dag(&mut self, u_hash: &H::Hash) {
        ...
        trace!(target: "BanklessBFT-terminal", "{:?} Inspecting parents for {:?}, missing {:?}", self.node_id, u_hash, u.n_miss_par_dag);
        if u.n_miss_par_dag == NodeCount(0) {
            self.event_queue
                .push_back(TerminalEvent::ParentsInDag(*u_hash));
        } else {
            ...
            match event {
                TerminalEvent::ParentsReconstructed(u_hash) => {
                    ...
                    if u.verify_control_hash() {
                        ...
                        warn!(target: "BanklessBFT-terminal", "{:?} wrong control hash", self.node_id);
                ...
                TerminalEvent::ParentsInDag(u_hash) => {
                    ...
                    trace!(target: "BanklessBFT-terminal", "{:?} Adding to Dag {:?} round {:?} index {:?}.", self.node_id, u_hash, u.unit.round(), u.unit.creator());
    ...
    fn send_notification(&mut self, notification: NotificationOut<H>) {
        if self.ntfct_tx.unbounded_send(notification).is_err() {
            warn!(target: "BanklessBFT-terminal", "{:?} Notification channel should be open", self.node_id);
            self.exiting = true;
        }
    }
    ...
    pub(crate) async fn run(&mut self, mut exit: oneshot::Receiver<()>) {
        loop {
            futures::select! {
                ...
                _ = &mut exit => {
                    info!(target: "BanklessBFT-terminal", "{:?} received exit signal", self.node_id);
                    self.exiting = true;
                }
            }
            if self.exiting {
                info!(target: "BanklessBFT-terminal", "{:?} Terminal decided to exit.", self.node_id);
                break;
            }
        }
    }
