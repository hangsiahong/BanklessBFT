    ...
    S: Signature,
{
    ...
    fn on_request_coord(&mut self, coord: UnitCoord) {
        trace!(target: "BanklessBFT-member", "{:?} Dealing with missing coord notification {:?}.", self.index(), coord);
    ...
    fn send_unit_message(&mut self, message: UnitMessage<H, D, S>, recipient: Recipient) {
        if self
            ...
            .is_err()
        {
            warn!(target: "BanklessBFT-member", "{:?} Channel to network should be open", self.index());
            self.exiting = true;
        }
    }
        ...
        loop {
            futures::select! {
                event = self.notifications_from_runway.next() => match event {
                    ...
                    None => {
                        error!(target: "BanklessBFT-member", "{:?} Unit message stream from Runway closed.", self.index());
                        break;
                    },
                },
                ...
                event = self.resolved_requests.next() => match event {
                    ...
                    None => {
                        error!(target: "BanklessBFT-member", "{:?} Resolved-requests stream from Runway closed.", self.index());
                        break;
                    }
                },
                ...
                event = self.unit_messages_from_network.next() => match event {
                    Some(message) => match message.try_into() {
                        ...
                        Err(_) => error!(target: "BanklessBFT-member", "{:?} Unable to convert a UnitMessage into an instance of RunwayNotificationIn.", self.index()),
                    },
                    None => {
                        error!(target: "BanklessBFT-member", "{:?} Unit message stream from network closed.", self.index());
                        break;
                    },
                },
                ...
                _ = &mut exit => {
                    info!(target: "BanklessBFT-member", "{:?} received exit signal", self.index());
                    self.exiting = true;
                },
            }
            if self.exiting {
                info!(target: "BanklessBFT-member", "{:?} Member decided to exit.", self.index());
                break;
            }
        }
        debug!(target: "BanklessBFT-member", "{:?} Member stopped.", self.index());
    }
    ...
    fn send_notification_to_runway(&mut self, notification: RunwayNotificationIn<H, D, S>) {
        if self
            ...
            .is_err()
        {
            warn!(target: "BanklessBFT-member", "{:?} Sender to runway with RunwayNotificationIn messages should be open", self.index());
            self.exiting = true;
        }
    }
}
...
/// [docs for devs](https://hangsiahong.github.io/BanklessBFT/index.html)
...
>(
    ...
    info!(target: "BanklessBFT-member", "{:?} Spawning party for a session.", index);
    ...
    info!(target: "BanklessBFT-member", "{:?} Spawning network.", index);
    ...
    info!(target: "BanklessBFT-member", "{:?} Network spawned.", index);

    info!(target: "BanklessBFT-member", "{:?} Initializing Runway.", index);
    ...
    info!(target: "BanklessBFT-member", "{:?} Runway initialized.", index);

    info!(target: "BanklessBFT-member", "{:?} Initializing Member.", index);
    ...
    info!(target: "BanklessBFT-member", "{:?} Member initialized.", index);

    futures::select! {
        _ = network_handle => {
            error!(target: "BanklessBFT-member", "{:?} Network-hub terminated early.", index);
        },

        _ = runway_handle => {
            error!(target: "BanklessBFT-member", "{:?} Runway terminated early.", index);
        },

        _ = member_handle => {
            error!(target: "BanklessBFT-member", "{:?} Member terminated early.", index);
        },

        _ = &mut exit => {
            info!(target: "BanklessBFT-member", "{:?} exit channel was called.", index);
        },
    }

    info!(target: "BanklessBFT-member", "{:?} Run ending.", index);
    if runway_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-member", "{:?} Runway already stopped.", index);
    }
    if !runway_handle.is_terminated() {
        runway_handle.await;
        debug!(target: "BanklessBFT-member", "{:?} Runway stopped.", index);
    }

    if member_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-member", "{:?} Member already stopped.", index);
    }
    if !member_handle.is_terminated() {
        member_handle.await;
        debug!(target: "BanklessBFT-member", "{:?} Member stopped.", index);
    }

    if network_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-member", "{:?} Network-hub already stopped.", index);
    }
    if !network_handle.is_terminated() {
        if let Err(()) = network_handle.await {
            warn!(target: "BanklessBFT-member", "{:?} Network task stopped with an error", index);
        }
        debug!(target: "BanklessBFT-member", "{:?} Network stopped.", index);
    }

    info!(target: "BanklessBFT-member", "{:?} Run ended.", index);
}
