...
pub(crate) async fn run<H: Hasher + 'static>(
    ...
    info!(target: "BanklessBFT", "{:?} Starting all services...", conf.node_ix);
    ...
    let mut terminal_handle = spawn_handle
        ...
        .fuse();
    info!(target: "BanklessBFT", "{:?} All services started.", index);

    futures::select! {
        _ = exit => {},
        _ = terminal_handle => {
            debug!(target: "BanklessBFT-consensus", "{:?} terminal task terminated early.", index);
        },
        _ = creator_handle => {
            debug!(target: "BanklessBFT-consensus", "{:?} creator task terminated early.", index);
        },
        _ = extender_handle => {
            debug!(target: "BanklessBFT-consensus", "{:?} extender task terminated early.", index);
        }
    }
    info!(target: "BanklessBFT", "{:?} All services stopping.", index);

    // we stop no matter if received Ok or Err
    if terminal_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-consensus", "{:?} terminal already stopped.", index);
    }
    if !terminal_handle.is_terminated() {
        if let Err(()) = terminal_handle.await {
            warn!(target: "BanklessBFT-consensus", "{:?} Terminal finished with an error", index);
        }
        debug!(target: "BanklessBFT-consensus", "{:?} terminal stopped.", index);
    }

    if creator_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-consensus", "{:?} creator already stopped.", index);
    }
    if !creator_handle.is_terminated() {
        if let Err(()) = creator_handle.await {
            warn!(target: "BanklessBFT-consensus", "{:?} Creator finished with an error", index);
        }
        debug!(target: "BanklessBFT-consensus", "{:?} creator stopped.", index);
    }

    if extender_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-consensus", "{:?} extender already stopped.", index);
    }
    if !extender_handle.is_terminated() {
        if let Err(()) = extender_handle.await {
            warn!(target: "BanklessBFT-consensus", "{:?} Extender finished with an error", index);
        }
        debug!(target: "BanklessBFT-consensus", "{:?} extender stopped.", index);
    }

    info!(target: "BanklessBFT", "{:?} All services stopped.", index);
}
