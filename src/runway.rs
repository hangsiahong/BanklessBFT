    ...
    FH: FinalizationHandler<D>,
{
    ...
    async fn on_unit_message(&mut self, message: RunwayNotificationIn<H, D, MK::Signature>) {
        match message {
            RunwayNotificationIn::NewUnit(u) => {
                trace!(target: "BanklessBFT-runway", "{:?} New unit received {:?}.", self.index(), &u);
                self.on_unit_received(u, false)
            }
            RunwayNotificationIn::Request(request, node_id) => match request {
                Request::Coord(coord) => {
                    trace!(target: "BanklessBFT-runway", "{:?} Coords request received {:?}.", self.index(), coord);
                    self.on_request_coord(node_id, coord)
                }
                Request::Parents(u_hash) => {
                    trace!(target: "BanklessBFT-runway", "{:?} Parents request received {:?}.", self.index(), u_hash);
                    self.on_request_parents(node_id, u_hash)
                }
                Request::NewestUnit(salt) => {
                    trace!(target: "BanklessBFT-runway", "{:?} Newest unit request received {:?}.", self.index(), salt);
                    self.on_request_newest(node_id, salt).await
                }
            },
            RunwayNotificationIn::Response(res) => match res {
                Response::Coord(u) => {
                    trace!(target: "BanklessBFT-runway", "{:?} Fetch response received {:?}.", self.index(), &u);
                    self.on_unit_received(u, false)
                }
                Response::Parents(u_hash, parents) => {
                    trace!(target: "BanklessBFT-runway", "{:?} Response parents received {:?}.", self.index(), u_hash);
                    self.on_parents_response(u_hash, parents)
                }
                Response::NewestUnit(response) => {
                    let salt = response.as_signable().salt;
                    trace!(target: "BanklessBFT-runway", "{:?} Response parents received {:?}.", self.index(), salt);
    ...
    fn validate_unit(
        ...
        let su = match uu.check(self.keybox) {
            Ok(su) => su,
            Err(uu) => {
                warn!(target: "BanklessBFT-runway", "{:?} Wrong signature received {:?}.", self.index(), &uu);
                return None;
            }
        };
        ...
        if full_unit.session_id() != self.session_id {
            ...
            warn!(target: "BanklessBFT-runway", "{:?} A unit with incorrect session_id! {:?}", self.index(), full_unit);
            return None;
        }
        if full_unit.round() > self.store.limit_per_node() {
            warn!(target: "BanklessBFT-runway", "{:?} A unit with too high round {}! {:?}", self.index(), full_unit.round(), full_unit);
            return None;
        }
        if full_unit.creator().0 >= self.n_members.0 {
            warn!(target: "BanklessBFT-runway", "{:?} A unit with too high creator index {}! {:?}", self.index(), full_unit.creator().0, full_unit);
            return None;
        }
        if !self.validate_unit_parents(&su) {
            warn!(target: "BanklessBFT-runway", "{:?} A unit did not pass parents validation. {:?}", self.index(), full_unit);
            return None;
        }
        Some(su)
    }
    ...
    fn add_unit_to_store_unless_fork(&mut self, su: SignedUnit<'a, H, D, MK>) {
        let full_unit = su.as_signable();
        trace!(target: "BanklessBFT-member", "{:?} Adding member unit to store {:?}", self.index(), full_unit);
        if self.store.is_forker(full_unit.creator()) {
            trace!(target: "BanklessBFT-member", "{:?} Ignoring forker's unit {:?}", self.index(), full_unit);
            return;
        }
    ...
    fn validate_unit_parents(&self, su: &SignedUnit<H, D, MK>) -> bool {
        ...
        if pre_unit.n_members() != self.n_members {
            warn!(target: "BanklessBFT-runway", "{:?} Unit with wrong length of parents map.", self.index());
            return false;
        }
        let round = pre_unit.round();
        let n_parents = pre_unit.n_parents();
        if round == 0 && n_parents > NodeCount(0) {
            warn!(target: "BanklessBFT-runway", "{:?} Unit of round zero with non-zero number of parents.", self.index());
            return false;
        }
        let threshold = self.threshold();
        if round > 0 && n_parents < threshold {
            warn!(target: "BanklessBFT-runway", "{:?} Unit of non-zero round with only {:?} parents while at least {:?} are required.", self.index(), n_parents, threshold);
            return false;
        }
        let control_hash = &pre_unit.control_hash();
        if round > 0 && !control_hash.parents_mask[pre_unit.creator()] {
            warn!(target: "BanklessBFT-runway", "{:?} Unit does not have its creator's previous unit as parent.", self.index());
            return false;
        }
        true
    }
    ...
    fn on_new_forker_detected(&mut self, forker: NodeIndex, proof: ForkProof<H, D, MK::Signature>) {
        ...
        if self.alerts_for_alerter.unbounded_send(alert).is_err() {
            warn!(target: "BanklessBFT-runway", "{:?} Channel to alerter should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn on_request_coord(&mut self, node_id: NodeIndex, coord: UnitCoord) {
        debug!(target: "BanklessBFT-runway", "{:?} Received fetch request for coord {:?} from {:?}.", self.index(), coord, node_id);
        let maybe_su = (self.store.unit_by_coord(coord)).cloned();

        if let Some(su) = maybe_su {
            trace!(target: "BanklessBFT-runway", "{:?} Answering fetch request for coord {:?} from {:?}.", self.index(), coord, node_id);
            self.send_message_for_network(RunwayNotificationOut::Response(
                Response::Coord(su.into()),
                node_id,
            ));
        } else {
            trace!(target: "BanklessBFT-runway", "{:?} Not answering fetch request for coord {:?}. Unit not in store.", self.index(), coord);
        }
    }

    fn on_request_parents(&mut self, node_id: NodeIndex, u_hash: H::Hash) {
        debug!(target: "BanklessBFT-runway", "{:?} Received parents request for hash {:?} from {:?}.", self.index(), u_hash, node_id);

        if let Some(p_hashes) = self.store.get_parents(u_hash) {
            let p_hashes = p_hashes.clone();
            trace!(target: "BanklessBFT-runway", "{:?} Answering parents request for hash {:?} from {:?}.", self.index(), u_hash, node_id);
            ...
            for hash in p_hashes.iter() {
                if let Some(fu) = self.store.unit_by_hash(hash) {
                    ...
                    debug!(target: "BanklessBFT-runway", "{:?} Not answering parents request, one of the parents missing from store.", self.index());
            ...
            trace!(target: "BanklessBFT-runway", "{:?} Not answering parents request for hash {:?}. Unit not in DAG yet.", self.index(), u_hash);
        }
    }
    ...
    async fn on_request_newest(&mut self, requester: NodeIndex, salt: u64) {
        ...
        {
            error!(target: "BanklessBFT-runway", "Unable to send response to network: {}", e);
        }
    }
    ...
    fn on_parents_response(
        ...
        if self.store.get_parents(u_hash).is_some() {
            trace!(target: "BanklessBFT-runway", "{:?} We got parents response but already know the parents.", self.index());
            return;
        }
        let (u_round, u_control_hash, parent_ids) = match self.store.unit_by_hash(&u_hash) {
            ...
            None => {
                trace!(target: "BanklessBFT-runway", "{:?} We got parents but don't even know the unit. Ignoring.", self.index());
                return;
            }
        };

        if parent_ids.len() != parents.len() {
            warn!(target: "BanklessBFT-runway", "{:?} In received parent response expected {} parents got {} for unit {:?}.", self.index(), parents.len(), parent_ids.len(), u_hash);
            return;
        }
        ...
        for (i, uu) in parents.into_iter().enumerate() {
            let su = match self.validate_unit(uu) {
                None => {
                    warn!(target: "BanklessBFT-runway", "{:?} In received parent response received a unit that does not pass validation.", self.index());
                    return;
                }
                Some(su) => su,
            };
            ...
            if full_unit.round() + 1 != u_round {
                warn!(target: "BanklessBFT-runway", "{:?} In received parent response received a unit with wrong round.", self.index());
                return;
            }
            if full_unit.creator() != parent_ids[i] {
                warn!(target: "BanklessBFT-runway", "{:?} In received parent response received a unit with wrong creator.", self.index());
                return;
            }
        ...
        if ControlHash::<H>::combine_hashes(&p_hashes_node_map) != u_control_hash {
            warn!(target: "BanklessBFT-runway", "{:?} In received parent response the control hash is incorrect {:?}.", self.index(), p_hashes_node_map);
            return;
        }
        let p_hashes: Vec<_> = p_hashes_node_map.into_values().collect();
        self.store.add_parents(u_hash, p_hashes.clone());
        trace!(target: "BanklessBFT-runway", "{:?} Succesful parents response for {:?}.", self.index(), u_hash);
    ...
    fn on_newest_response(
        ...
        if self.starting_round_sender.is_none() {
            log::debug!(target: "BanklessBFT-member", "Starting round already sent, ignoring newest unit response");
            return;
        }
        let response = match unchecked_response.check(self.keybox) {
            Ok(checked) => checked.into_signable(),
            Err(e) => {
                log::debug!(target: "BanklessBFT-member", "incorrectly signed response: {:?}", e);
                return;
            }
        };

        if response.salt != self.salt {
            debug!(target: "BanklessBFT-member", "Ignoring newest unit response with an unknown salt: {:?}", response);
            return;
        }
        ...
        if let Some(unchecked_unit) = response.unit {
            let checked_unit = match self.validate_unit(unchecked_unit) {
                Some(unit) => unit,
                None => {
                    log::debug!(target: "BanklessBFT-member", "ivalid unit in response");
                    return;
                }
            };
            if checked_unit.as_signable().creator() != self.index() {
                log::debug!(target: "BanklessBFT-member", "Not our unit in a response:  {:?}", checked_unit.into_signable());
                return;
            }
            if self
                .store
    ...
    fn resolve_starting_round(&mut self) {
        let starting_round_sender = match self.starting_round_sender.take() {
            Some(sender) => sender,
            None => {
                warn!(target: "BanklessBFT-runway", "Trying to resolve starting round after resolving it earlier");
                return;
            }
        };
        if starting_round_sender
            ...
            .is_err()
        {
            error!(target: "BanklessBFT-runway", "unable to send starting round to creator");
            self.exiting = true;
            return;
        }
        if let Err(e) = self
        ...
        {
            error!(target: "BanklessBFT-runway", "unable to send resolved request:  {}", e);
        }
    }
    ...
    async fn on_create(&mut self, u: PreUnit<H>) {
        debug!(target: "BanklessBFT-runway", "{:?} On create notification.", self.index());
            ...
            NotificationOut::AddedToDag(h, p_hashes) => {
                ...
                if let Some(su) = self.store.unit_by_hash(&h).cloned() {
                    if su.as_signable().creator() == self.index() {
                        trace!(target: "BanklessBFT-runway", "{:?} Sending a unit {:?}.", self.index(), h);
                        self.send_message_for_network(RunwayNotificationOut::NewUnit(su.into()));
                    }
                } else {
                    error!(target: "BanklessBFT-runway", "{:?} A unit already added to DAG is not in our store: {:?}.", self.index(), h);
                }
            }
        }
    }
    ...
    fn on_missing_coords(&mut self, mut coords: Vec<UnitCoord>) {
        trace!(target: "BanklessBFT-runway", "{:?} Dealing with missing coords notification {:?}.", self.index(), coords);
    ...
    fn on_wrong_control_hash(&mut self, u_hash: H::Hash) {
        trace!(target: "BanklessBFT-runway", "{:?} Dealing with wrong control hash notification {:?}.", self.index(), u_hash);
        if let Some(p_hashes) = self.store.get_parents(u_hash) {
            // We have the parents by some strange reason (someone sent us parents
            // without us requesting them).
            let p_hashes = p_hashes.clone();
            trace!(target: "BanklessBFT-runway", "{:?} We have the parents for {:?} even though we did not request them.", self.index(), u_hash);
    ...
    fn send_message_for_network(
        ...
        if self
            ...
            .is_err()
        {
            warn!(target: "BanklessBFT-runway", "{:?} unit_messages_for_network channel should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn send_resolved_request_notification(&mut self, notification: Request<H>) {
        if self.resolved_requests.unbounded_send(notification).is_err() {
            warn!(target: "BanklessBFT-runway", "{:?} resolved_requests channel should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn send_consensus_notification(&mut self, notification: NotificationIn<H>) {
        if self.tx_consensus.unbounded_send(notification).is_err() {
            warn!(target: "BanklessBFT-runway", "{:?} Channel to consensus should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    async fn run(mut self, mut exit: oneshot::Receiver<()>) {
        ...
        info!(target: "BanklessBFT-runway", "{:?} Runway starting.", index);
        ...
        if let Err(e) = self.unit_messages_for_network.unbounded_send(notification) {
            error!(target: "BanklessBFT-runway", "{:?} Unable to send the newest unit request: {}", index, e);
            self.exiting = true
        };

        let mut catch_up_delay = futures_timer::Delay::new(Duration::from_secs(5)).fuse();

        info!(target: "BanklessBFT-runway", "{:?} Runway started.", index);
        loop {
            futures::select! {
                notification = self.rx_consensus.next() => match notification {
                        ...
                        None => {
                            error!(target: "BanklessBFT-runway", "{:?} Consensus notification stream closed.", index);
                            break;
                        }
                },
                ...
                notification = self.notifications_from_alerter.next() => match notification {
                    ...
                    None => {
                        error!(target: "BanklessBFT-runway", "{:?} Alert notification stream closed.", index);
                        break;
                    }
                },
                ...
                event = self.unit_messages_from_network.next() => match event {
                    ...
                    None => {
                        error!(target: "BanklessBFT-runway", "{:?} Unit message stream closed.", index);
                        break;
                    }
                },
                ...
                batch = self.ordered_batch_rx.next() => match batch {
                    ...
                    None => {
                        error!(target: "BanklessBFT-runway", "{:?} Ordered batch stream closed.", index);
                        break;
                    }
                },
                ...
                _ = &mut exit => {
                    info!(target: "BanklessBFT-runway", "{:?} received exit signal", self.index());
                    self.exiting = true;
                }
            };
            ...
            if self.exiting {
                info!(target: "BanklessBFT-runway", "{:?} Runway decided to exit.", index);
                break;
            }
        }

        info!(target: "BanklessBFT-runway", "{:?} Run ended.", index);
    }
}
    ...
    SH: SpawnHandle,
{
    ...
    futures::select! {
        _ = runway_handle => {
            debug!(target: "BanklessBFT-runway", "{:?} Runway task terminated early.", index);
        },
        _ = alerter_handle => {
            debug!(target: "BanklessBFT-runway", "{:?} Alerter task terminated early.", index);
        },
        _ = consensus_handle => {
            debug!(target: "BanklessBFT-runway", "{:?} Consensus task terminated early.", index);
        },
        _ = &mut exit => {},
    }

    info!(target: "BanklessBFT-runway", "{:?} Ending run.", index);

    if consensus_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-runway", "{:?} Consensus already stopped.", index);
    }
    if !consensus_handle.is_terminated() {
        if let Err(()) = consensus_handle.await {
            warn!(target: "BanklessBFT-runway", "{:?} Consensus finished with an error", index);
        }
        debug!(target: "BanklessBFT-runway", "{:?} Consensus stopped.", index);
    }

    if alerter_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-runway", "{:?} Alerter already stopped.", index);
    }
    if !alerter_handle.is_terminated() {
        if let Err(()) = alerter_handle.await {
            warn!(target: "BanklessBFT-runway", "{:?} Alerter finished with an error", index);
        }
        debug!(target: "BanklessBFT-runway", "{:?} Alerter stopped.", index);
    }

    if runway_exit.send(()).is_err() {
        debug!(target: "BanklessBFT-runway", "{:?} Runway already stopped.", index);
    }
    if !runway_handle.is_terminated() {
        runway_handle.await;
        debug!(target: "BanklessBFT-runway", "{:?} Runway stopped.", index);
    }

    info!(target: "BanklessBFT-runway", "{:?} Runway ended.", index);
}
