...
/// of all honest nodes. The static Bankless Consensus algorithm is then run on this Dag in order
...
///
/// We refer to the documentation https://hangsiahong.github.io/BanklessBFT/internals.html
...
impl<H: Hasher> Extender<H> {
    ...
    fn add_unit(&mut self, u: ExtenderUnit<H>) {
        debug!(target: "BanklessBFT-extender", "{:?} New unit in Extender round {:?} creator {:?} hash {:?}.", self.node_id, u.round, u.creator, u.hash);
        let round = u.round;
    ...
    fn finalize_round(&mut self, round: Round, head: &H::Hash) {
        ...
        if self.finalizer_tx.unbounded_send(batch).is_err() {
            warn!(target: "BanklessBFT-extender", "{:?} Channel for batches should be open", self.node_id);
            self.exiting = true;
        }

        debug!(target: "BanklessBFT-extender", "{:?} Finalized round {:?} with head {:?}.", self.node_id, round, head);
    ...
    pub(crate) async fn extend(&mut self, mut exit: oneshot::Receiver<()>) {
        loop {
            futures::select! {
                ...
                _ = &mut exit => {
                    info!(target: "BanklessBFT-extender", "{:?} received exit signal.", self.node_id);
                    self.exiting = true;
                }
            }
            if self.exiting {
                info!(target: "BanklessBFT-extender", "{:?} Extender decided to exit.", self.node_id);
                break;
            }
        }
    }
